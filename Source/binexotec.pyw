...
class Application(Frame):
    ...
    def create_layout(self):
        ...
        space1 = Frame(space0, bg="#5A0001")
        space1.grid(row=0, column=0, padx=2, pady=2, sticky='nsew')
        space2 = Frame(space0, bg="#5A0001")
        space2.grid(row=2, column=0, padx=2, pady=2, sticky='nsew')
        ...
        for idx, label in enumerate(operations):
            self.create_entry_section1(space2, row=idx, column=0, label_text=label, var_name=f"{label.lower()}_var")
    ...
    def create_entry_section(self, parent, row, label_text, var_name):
        ...
        # Reemplazar Label por Button
        button = Button(parent,width=1, text=label_text, bg="silver", command=lambda: self.on_button_click(label_text))
        button.grid(row=row, column=0, padx=2, pady=2, sticky='wnse')

        entry_var = StringVar()
        entry = Entry(parent,width=46, textvariable=entry_var, font=("Consolas", 9), fg="white", bg="#3E3E3E", insertbackground="white")
